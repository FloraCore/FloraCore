...
final class FieldWriter extends FieldVisitor {
	...
	/**
	 * The first nonstandard attribute of this field. The next ones can be accessed with the {@link
	...
	static int getAttributesCount(boolean useSyntheticAttribute, int attributesCount, int accessFlags, int signatureIndex, AnnotationWriter lastRuntimeVisibleAnnotation, AnnotationWriter lastRuntimeInvisibleAnnotation, boolean b, boolean b2) {
	...
	void putFieldInfo(final ByteVector output) {
		...
		// For ease of reference, we use here the same attribute order as in Section 4.7 of the JVMS.
		int attributesCount = getAttributesCount(useSyntheticAttribute);
	...
	private int getAttributesCount(boolean useSyntheticAttribute) {
		int attributesCount = 0;
		if (constantValueIndex != 0) {
			++attributesCount;
		}
		attributesCount = getAttributesCount(useSyntheticAttribute, attributesCount, accessFlags, signatureIndex, lastRuntimeVisibleAnnotation, lastRuntimeInvisibleAnnotation, lastRuntimeVisibleTypeAnnotation != null, lastRuntimeInvisibleTypeAnnotation != null);
		if (firstAttribute != null) {

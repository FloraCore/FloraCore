...
public class PARTY extends AbstractFloraCoreTable {
    ...
    private static final String INSERT = "INSERT INTO '{prefix}party' (uuid, leader, members, settings, createTime, disbandTime, chat) VALUES(?, ?, ?, ?, ?, ?, ?)";
    ...
    public PARTY(FloraCorePlugin plugin, StorageImplementation storageImplementation, int id, UUID uuid, UUID leader, List<UUID> members, String settings, long createTime, long disbandTime, int chat) {
    ...
    public void init() throws SQLException {
        ...
        try (Connection connection = getStorageImplementation().getConnectionFactory().getConnection()) {
            try (PreparedStatement ps = connection.prepareStatement(getStorageImplementation().getStatementProcessor().apply(INSERT))) {
                ...
                ps.setString(4, settings);
                ps.setLong(5, createTime);
                ps.setLong(6, disbandTime);
                ps.setInt(7, chat);
                ps.execute();
            }
        }

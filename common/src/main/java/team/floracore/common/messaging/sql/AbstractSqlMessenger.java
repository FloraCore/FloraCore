...
public abstract class AbstractSqlMessenger implements Messenger {
    ...
    public void init() throws SQLException {
        try (Connection c = getConnection()) {
            // init table
            String createStatement = "CREATE TABLE IF NOT EXISTS `" + getTableName() + "` (`id` INT AUTO_INCREMENT NOT" +
                    " NULL, `time` TIMESTAMP NOT NULL, `msg` TEXT NOT NULL, PRIMARY KEY (`id`)) DEFAULT CHARSET = " +
                    "utf8mb4";
    ...
    public void sendOutgoingMessage(@NotNull OutgoingMessage outgoingMessage) {
        ...
        try (Connection c = getConnection()) {
            try (PreparedStatement ps = c.prepareStatement("INSERT INTO `" + getTableName() + "` (`time`, `msg`) " +
                    "VALUES(NOW(), ?)")) {
    ...
    public void pollMessages() {
        ...
        try (Connection c = getConnection()) {
            try (PreparedStatement ps = c.prepareStatement("SELECT `id`, `msg` FROM `" + getTableName() + "` WHERE " +
                    "`id` > ? AND (NOW() - `time` < 30)")) {
    ...
    public void runHousekeeping() {
        ...
        try (Connection c = getConnection()) {
            try (PreparedStatement ps = c.prepareStatement("DELETE FROM `" + getTableName() + "` WHERE (NOW() - `time`" +
                    " > 60)")) {
                ps.execute();
            }
        } catch (SQLException e) {

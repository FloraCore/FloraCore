					...
					switch (i.getKey().location()) {
						...
						case FRONT:
								...
								for (; loc < raw.size(); loc++) {
									if (raw.get(loc).getOpcode() == i.getKey().byteCode().opcode()) {
										if (raw.get(loc) instanceof MethodInsnNode) {
											...
											if (i.getKey().byteCode().label() != labels.indexOf(((JumpInsnNode) raw.get(
													loc)).label.getLabel())) {
												continue;
											}
										}
							...
							if (i.getValue().getDeclaredAnnotation(ManualByteCode.class) != null) {
								...
								node.instructions.insertBefore(raw.get(loc), injectNode(i.getValue(),
										new HashMap<>()));
							}
							break;
						case RETURN:
	...
	public InsnList injectNode(Method method,
		...
		for (int i = 0; i < pas.length; i++) {
			for (Annotation a : pas[i]) {
				L1:
				{
					for (Map.Entry<Class<? extends Annotation>, BiFunction<Annotation, Class<?>, InsnList>> p :
							argNodeParsers.entrySet()) {

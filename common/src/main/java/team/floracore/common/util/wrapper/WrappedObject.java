...
import team.floracore.common.util.SimpleLoader;
        ...
        try {
            ...
            if (c == null) {
                ...
                if (!WrappedObject.class.isAssignableFrom(wrapper)) {
                    throw new IllegalArgumentException("Wrapper " + wrapper.getName() + " must be a subclass of " +
                            "WrappedObject.");
                }
                ...
                if (rawClass == null) {
                    if (wrapper.getDeclaredAnnotation(Optional.class) == null) {
                        throw new IllegalArgumentException("Wrapper " + wrapper.getName() + " must annotate its raw " +
                                "class.");
                    }
                    return null;
                }
                ...
                try {
                    ...
                    cn.visit(Opcodes.V1_8,
                            Opcodes.ACC_PUBLIC,
                            AsmUtil.getType("mz.lib.wrapper.gen." + wrapper.getName()),
                            null,
                            getType(AbsWrappedObject.class),
                            new String[]{getType(wrapper)});

                    cn.fields.add(new FieldNode(Opcodes.ACC_PUBLIC | Opcodes.ACC_STATIC,
                            "objects",
                            getDesc(Object[].class),
                            null,
                            null));
                    ...
                    MethodNode mn = new MethodNode(Opcodes.ACC_PUBLIC,
                            "<init>",
                            getDesc(new Class[]{Object.class}, void.class),
                            null,
                            new String[0]);
                    ...
                    mn = new MethodNode(Opcodes.ACC_PUBLIC,
                            "getRawClass",
                            getDesc(new Class[0], Class.class),
                            null,
                            new String[0]);
                    ...
                    for (Method m : wrapper.getDeclaredMethods()) {
                        ...
                        boolean accessByForce =
                                m.getDeclaredAnnotation(AccessByForce.class) != null || (!Modifier.isPublic(
                        ...
                        if (rm instanceof Field) {
                            switch (m.getParameterCount()) {
                                case 0:
                                    ...
                                    if (accessByForce) {
                                        mn.visitFieldInsn(Opcodes.GETSTATIC,
                                                ...
                                                "objects",
                                                getDesc(Object[].class));
                                        mn.instructions.add(arrayLoadNode(Object.class,
                                                toList(ldcNode(objects.size()))));
                                        ...
                                        mn.visitMethodInsn(Opcodes.INVOKEVIRTUAL,
                                                ...
                                                "invokeExact",
                                                getDesc(Modifier.isStatic(rm.getModifiers()) ? new Class[0] :
                                                                new Class[]{rm.getDeclaringClass()},
                                                        ((Field) rm).getType()),
                                                false);
                                        if (WrappedObject.class.isAssignableFrom(m.getReturnType())) {
                                        ...
                                        mn.visitFieldInsn(Modifier.isStatic(rm.getModifiers()) ? Opcodes.GETSTATIC :
                                                        Opcodes.GETFIELD,
                                                getType(rm.getDeclaringClass()),
                                                rm.getName(),
                                                getDesc(((Field) rm).getType()));
                                        if (WrappedObject.class.isAssignableFrom(m.getReturnType())) {
                                            mn.instructions.add(castNode(Object.class, ((Field) rm).getType()));
                                            mn.instructions.add(wrapNode());
                                            mn.instructions.add(castNode(m.getReturnType(), WrappedObject.class));
                                ...
                                case 1:
                                    if (accessByForce || Modifier.isFinal(rm.getModifiers())) {
                                        ...
                                        if (WrappedObject.class.isAssignableFrom(m.getParameterTypes()[0])) {
                                            ...
                                            mn.instructions.add(AsmUtil.castNode(((Field) rm).getType(),
                                                    Object.class));
                                        } else {
                                            mn.instructions.add(AsmUtil.castNode(((Field) rm).getType(),
                                        ...
                                        mn.visitMethodInsn(Opcodes.INVOKEVIRTUAL,
                                                getType(MethodHandle.class),
                                                "invokeExact",
                                                getDesc(Modifier.isStatic(rm.getModifiers()) ?
                                                                new Class[]{((Field) rm).getType()} :
                                                                new Class[]{rm.getDeclaringClass(),
                                                                        ((Field) rm).getType()},
                                                        void.class),
                                                false);
                                        if (m.getReturnType() == void.class) {
                                            ...
                                            throw new IllegalArgumentException(
                                                    "Return type of setter must be This or void. On " + m);
                                        }
                                        ...
                                        if (!Modifier.isStatic(rm.getModifiers())) {
                                            mn.visitVarInsn(Opcodes.ALOAD, 0);
                                            mn.instructions.add(getRawNode());
                                            mn.instructions.add(castNode(((Field) rm).getDeclaringClass(),
                                                    Object.class));
                                        }
                                        mn.instructions.add(AsmUtil.varLoadNode(m.getParameterTypes()[0], 1));
                                        if (WrappedObject.class.isAssignableFrom(m.getParameterTypes()[0])) {
                                        ...
                                        mn.visitFieldInsn(Modifier.isStatic(rm.getModifiers()) ? Opcodes.PUTSTATIC :
                                                        Opcodes.PUTFIELD,
                                ...
                                default:
                                    throw new IllegalArgumentException(
                                            "Field accessor can only be getter(void) or setter(value). On " + m + ".");
                            }
                            ...
                            if (accessByForce) {
                                ...
                                for (int i = 0; i < m.getParameterCount(); i++) {
                                    ...
                                    if (WrappedObject.class.isAssignableFrom(m.getParameterTypes()[i])) {
                                        mn.instructions.add(AsmUtil.getRawNode());
                                        mn.instructions.add(AsmUtil.castNode(((Constructor<?>) rm).getParameterTypes()[i],
                                                Object.class));
                                    } else {
                                        mn.instructions.add(AsmUtil.castNode(((Constructor<?>) rm).getParameterTypes()[i],
                                ...
                                mn.visitMethodInsn(Opcodes.INVOKEVIRTUAL,
                                        getType(MethodHandle.class),
                                        "invokeExact",
                                        getDesc(((Constructor<?>) rm).getParameterTypes(), rawClass),
                                        false);
                            ...
                            if (accessByForce) {
                                ...
                                for (int i = 0; i < m.getParameterCount(); i++) {
                                    ...
                                    if (WrappedObject.class.isAssignableFrom(m.getParameterTypes()[i])) {
                                        mn.instructions.add(AsmUtil.getRawNode());
                                        mn.instructions.add(AsmUtil.castNode(((Method) rm).getParameterTypes()[i],
                                                Object.class));
                                    } else {
                                        mn.instructions.add(AsmUtil.castNode(((Method) rm).getParameterTypes()[i],
                                                m.getParameterTypes()[i]));
                                ...
                                mn.visitMethodInsn(Opcodes.INVOKEVIRTUAL,
                                        ...
                                        "invokeExact",
                                        getDesc(Modifier.isStatic(rm.getModifiers()) ?
                                                        ((Method) rm).getParameterTypes() : ListUtil.mergeLists(
                                ...
                                mn.visitMethodInsn(Modifier.isStatic(rm.getModifiers()) ? Opcodes.INVOKESTATIC :
                                                (rm.getDeclaringClass()
                                                        .isInterface() ? Opcodes.INVOKEINTERFACE : Opcodes.INVOKEVIRTUAL),
                                        AsmUtil.getType(rm.getDeclaringClass()),
                                        rm.getName(),
                                        getDesc((Method) rm),
                                        rm.getDeclaringClass().isInterface());
                                if (WrappedObject.class.isAssignableFrom(m.getReturnType())) {
                                    mn.instructions.add(castNode(Object.class, ((Method) rm).getReturnType()));
                                    mn.instructions.add(wrapNode());
                                    mn.instructions.add(castNode(m.getReturnType(), WrappedObject.class));
                    ...
                    for (Method m : wrapper.getMethods()) {
                        if (!Modifier.isAbstract(m.getModifiers()) || Modifier.isStatic(m.getModifiers()) || !WrappedObject.class.isAssignableFrom(
                                m.getDeclaringClass())) {
                            continue;
                        }
                        ...
                        if (m.getName().equals("getRaw") && m.getParameterCount() == 0) {
                            ...
                            mn.visitMethodInsn(Opcodes.INVOKESPECIAL,
                                    getType(AbsWrappedObject.class),
                                    "getRaw",
                                    getDesc(new Class[0], Object.class),
                                    false);
                    ...
                    cn.accept(cw);
                    Class<?> cl = ClassUtil.loadClass(cn.name,
                            cw.toByteArray(),
                            new SimpleLoader(wrapper.getClassLoader()));
                    try {
                        c = Root.getTrusted()
                                .findConstructor(cl, MethodType.methodType(void.class, new Class[]{Object.class}));

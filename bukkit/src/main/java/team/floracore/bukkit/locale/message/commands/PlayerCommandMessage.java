...
public interface PlayerCommandMessage extends AbstractMessage {
    Args2<Boolean, String> COMMAND_FLY = (status, target) -> AbstractMessage.prefixed(translatable()
            ...
            // 开 / 关
            .args(translatable(status ? "floracore.command.misc.on" : "floracore.command.misc.off").color(GREEN),
                    text(target).color(RED)).append(FULL_STOP));
    ...
    Args2<Component, String> COMMAND_GAMEMODE_FROM = (mode, from) -> AbstractMessage.prefixed(translatable()
            ...
            .color(AQUA)
            .args(mode.color(GREEN), text(from).color(GREEN))
            .append(FULL_STOP));
    ...
    Args1<String> COMMAND_MISC_NICK_RANK_UNKNOWN = (rank) -> AbstractMessage.prefixed(translatable()
            ...
            // {0}
            .args(translatable("floracore.command.misc.nick.book.rank-page.rank"),
                    text(rank).color(DARK_RED).decoration(BOLD, true)).color(RED));
    ...
    Args1<String> COMMAND_ITEMNAME_NOITEM_OTHER = player -> AbstractMessage.prefixed(translatable().key(
            "floracore.command.itemname.noitem.other").color(RED).args(text(player).color(GREEN)));

    Args0 COMMAND_ITEMNAME_UNSUPPORTED_ITEM = () -> AbstractMessage.prefixed(translatable().key(
            "floracore.command.itemname.unsupported-item").color(RED));

    Args1<String> COMMAND_ITEMNAME_SET_SELF = name -> AbstractMessage.prefixed(translatable().key(
    ...
    Args0 COMMAND_ITEMNAME_RESET_SELF = () -> AbstractMessage.prefixed(translatable().key(
            "floracore.command.itemname.reset.self").color(AQUA));

    Args1<String> COMMAND_ITEMNAME_RESET_OTHER = player -> AbstractMessage.prefixed(translatable().key(
            "floracore.command.itemname.reset.other").color(AQUA).args(text(player).color(GREEN)));

    Args1<String> COMMAND_ITEMNAME_RESET_FROM = from -> AbstractMessage.prefixed(translatable().key(
            "floracore.command.itemname.reset.from").color(AQUA).args(text(from).color(GREEN)));

    Args1<String> COMMAND_GIVE_ITEM_NOSUCH = itemKey -> AbstractMessage.prefixed(translatable().key(
            "floracore.command.give.item.nosuch").color(RED).args(text(itemKey).color(YELLOW)));

    Args1<String> COMMAND_GIVE_ITEM_NODATA = itemKey -> AbstractMessage.prefixed(translatable().key(
            "floracore.command.give.item.nodata").color(RED).args(text(itemKey).color(YELLOW)));

    Args0 COMMAND_GIVE_ITEM_NBT_SYNTAX_EXCEPTION = () -> AbstractMessage.prefixed(translatable().key(
            "floracore.command.give.item.nbt-syntax-exception").color(RED));

    Args2<String, String> COMMAND_GIVE_ITEM_GIVEN = (item, player) -> AbstractMessage.prefixed(translatable().key(
            "floracore.command.give.item.given").color(AQUA).args(
            text(item).color(WHITE),
            text(player).color(GREEN)
    ));

    Args1<String> COMMAND_MISC_REPORT_NOTICE_ACCEPTED = target -> AbstractMessage.prefixed(translatable().key(
            "floracore.command.misc.report.notice.accepted").color(AQUA).args(text(target).color(RED)));

    Args1<String> COMMAND_MISC_REPORT_NOTICE_PROCESSED = (target) -> AbstractMessage.prefixed(translatable().key(
